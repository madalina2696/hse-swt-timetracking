...
public class SupervisorController {
    ...
    public SupervisorController() throws IOException {
        ...
        for (Worker worker : dieBeschaeftigten.getEmployeeList().values()) {
            for (FlexTime tag : dieBeschaeftigten.getEmployeeList().get(worker.getId()).getFlexTimeAnfragen()
                    .values()) {
                if (!tag.getApproved()) {
    ...
    void btnFlextimeAnfrageAblehnenClicked(ActionEvent event) {
        ...
        try {
            lblFlextimeAnfragenEmp1.setText(
                    dieBeschaeftigten.getWorker(flextimeListe.get(goThroughFlextimes).getId()).getFirstName() + ' '
                            + dieBeschaeftigten.getWorker(flextimeListe.get(goThroughFlextimes).getId()).getLastName()
                            + " (" + flextimeListe.get(goThroughFlextimes).getId() + ')');
            lblFlextimeAnfragenEmp1Begin
                    .setText(flextimeListe.get(goThroughFlextimes).getFlextime().getDate().toString());
            lblFlextimeAnfragenEmp1Min.setText(flextimeListe.get(goThroughFlextimes).getFlextime().getMinute() + "");
    ...
    void btnFlextimeAnfrageAkzeptierenClicked(ActionEvent event) {
        if (firstTimeFlexTime) {
            ...
            try {
                dieBeschaeftigten.addFlexTimeToWorker(flextimeListe.get(goThroughFlextimes).getId(),
    ...
    void btnUrlaubsanfrageAblehnenClicked(ActionEvent event) {
        ...
        try {
            lblUrlaubsanfragenEmp1.setText(
                    dieBeschaeftigten.getWorker(urlaubsListe.get(goThroughVacations).getId()).getFirstName() + ' '
                            + dieBeschaeftigten.getWorker(urlaubsListe.get(goThroughVacations).getId()).getLastName()
            ...
            lblUrlaubsanfragenEmp1Min.setText(urlaubsListe.get(goThroughVacations).getHoliday().getMinute() + "");
    ...
    void btnUrlaubsanfrageAkzeptierenClicked(ActionEvent event) {
        if (firstTimeVaction) {
            ...
            try {
                dieBeschaeftigten.addVacationToWorker(urlaubsListe.get(goThroughVacations).getId(),
                        urlaubsListe.get(0).getHoliday().getDate().toString(),
                        urlaubsListe.get(0).getHoliday().getMinute() + "", "true");
                export();
                urlaubsListe.clear();
                for (Worker worker : dieBeschaeftigten.getEmployeeList().values()) {
                    for (Holiday tag : dieBeschaeftigten.getEmployeeList().get(worker.getId()).getUrlaubsAnfragen()
                            .values()) {
                        if (!tag.getApproved()) {
                            urlaubsListe.add(new IDandHoliday(worker.getId(), tag));
                        }
